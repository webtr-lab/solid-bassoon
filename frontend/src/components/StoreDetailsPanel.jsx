 ...
 */
export default function StoreDetailsPanel({ store, onClose }) {
  if (!store) return null;

  return (
    <div className="w-96 bg-white shadow-2xl border-l overflow-y-auto">
      {/* Header */}
      ...
      <div className="p-6 space-y-6">
        ...
        {(store.created_at || store.updated_at) && (
          <div className="pt-4 border-t text-xs text-gray-500 space-y-1">
            {store.created_at && (
              <div>Added: {new Date(store.created_at).toLocaleDateString()}</div>
            )}
            {store.updated_at && (
