 ...
 * Displays visit reports with filtering and CSV export
 */
function ReportsManagement(): JSX.Element {
  ...
  const fetchReports = async (): Promise<void> => {
    ...
    try {
      ...
      const data = await apiFetch<{ results: ReportResult[] }>(`/api/reports/visits?${params.toString()}`);
              ...
              <div>
                <div className="text-2xl font-bold text-blue-700">
                  {(() => {
                    ...
                    results.forEach(r => {
                      if (r.vehicles) {
                        r.vehicles.forEach(v => uniqueVisitors.add(`${v.type || 'vehicle'}_${v.id}`));
                      }
                    });
